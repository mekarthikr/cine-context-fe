...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const popularResponse = await fetch(`https://api.themoviedb.org/3/movie/popular?api_key=${API_KEY}&language=en-US&page=1`);
        ...
        const topRatedResponse = await fetch(`https://api.themoviedb.org/3/movie/top_rated?api_key=${API_KEY}&language=en-US&page=1`);
        ...
        const upcomingResponse = await fetch(`https://api.themoviedb.org/3/movie/upcoming?api_key=${API_KEY}&language=en-US&page=1`);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      {/* Navigation */}
      <nav className="sticky top-0 z-50 border-b border-slate-800/50 bg-slate-950/95 backdrop-blur-md" style={{borderBottomWidth:0}}>
          ...
          <div className="flex items-center justify-between">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="text-white hover:bg-slate-800/50 rounded-xl">
                  <User className="h-6 w-6" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="bg-slate-800/95 backdrop-blur-md border-slate-700/50 rounded-xl">
                <DropdownMenuItem className="text-white hover:bg-slate-700/50 rounded-lg">Profile</DropdownMenuItem>
                <DropdownMenuItem className="text-white hover:bg-slate-700/50 rounded-lg">Watchlist</DropdownMenuItem>
                <DropdownMenuItem className="text-white hover:bg-slate-700/50 rounded-lg">Settings</DropdownMenuItem>
                <DropdownMenuItem className="text-white hover:bg-slate-700/50 rounded-lg">Sign Out</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        </div>
      ...
      <section className="py-20 px-6 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950">
        <div className="container mx-auto text-center">
          <h1 className="text-6xl md:text-7xl font-bold mb-8 bg-gradient-to-r from-white via-purple-200 to-pink-200 bg-clip-text text-transparent leading-tight">
            Discover Stories<br />That Feel Right
          </h1>
          ...
          <div className="mb-16">
            <Select>
              ...
              <SelectContent className="bg-slate-800/95 backdrop-blur-md border-slate-700/50 rounded-xl">
                ...
                <SelectItem value="need-motivation" className="text-white hover:bg-slate-700/50 py-3">
                  💪 When you need motivation
                </SelectItem>
                <SelectItem value="feeling-nostalgic" className="text-white hover:bg-slate-700/50 py-3">
              ...
              featuredTitles.map((title) => (
                ...
                >
                  ...
                  <CardContent className="p-6" style={{paddingTop:0}}>
                    <h3 className="font-semibold text-white mb-3 text-lg" style={{textAlign:'start'}}>{title.title}</h3>
                    <div className="flex flex-wrap gap-2">
                      {title.tags.map((tag) => (
                        <Badge key={tag} className="bg-purple-500/20 text-purple-300 border-purple-500/30 px-3 py-1 rounded-full">
                          {tag}
                        </Badge>
                      ))}
                    </div>
      ...
      <div className="container mx-auto px-6 pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
                ...
                {["Emotions", "Themes", "Life Moments", "Character Arcs"].map((category, idx) => {
                  ...
                  return (
                    <div key={category} className="mb-8">
                      <h4 className="text-sm font-medium text-slate-300 mb-4 text-purple-300">{category}</h4>
                      <div className="space-y-3">
                        {items.map((item) => (
                          <div key={item} className="flex items-center space-x-3">
                            <Checkbox id={item} className="border-slate-600 data-[state=checked]:bg-purple-500 data-[state=checked]:border-purple-500" />
                            <label htmlFor={item} className="text-sm text-slate-300 hover:text-white transition-colors cursor-pointer">
                              {item}
                            </label>
                          </div>
                        ))}
                  ...
                  >
                    ...
                    <CardContent className="p-6" style={{paddingTop:1}}>
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <span className="text-sm text-slate-400 bg-slate-700/50 px-2 py-1 rounded-full">{movie.year}</span>
                      </div>

                      <div className="flex items-center gap-6 mb-4 text-sm text-slate-400">
                        <div className="flex items-center gap-2">
                          <Star className="h-4 w-4 fill-yellow-400 text-yellow-400" />
                          <span className="text-yellow-400 font-medium">{Number(movie.rating).toFixed(1)}</span>
                        </div>
                        <div className="flex items-center gap-2">
          ...
          <div className="lg:col-span-1">
            <Card style={{paddingTop:2,paddingBottom:2}} className="bg-slate-800/30 border-slate-700/30 backdrop-blur-sm rounded-2xl sticky top-24">
              <CardContent className="p-4">
                  ...
                  {isLoading ? (
                    ...
                    recommendations.map((rec, index) => (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <h4 className="text-sm font-medium text-white mb-2 leading-tight">{rec.title}</h4>
                          <div className="flex flex-wrap gap-1">
                            {rec.tags.map((tag) => (
                              <Badge key={tag} className="bg-slate-600/50 text-slate-300 text-xs px-2 py-1 rounded-full">
                                {tag}
                              </Badge>
                            ))}
                          </div>
                ...
                <div className="mt-10">
                  ...
                  <div className="space-y-4">
                    {[
                      ...
                      <div key={mood} className="flex justify-between items-center p-3 hover:bg-slate-700/30 rounded-lg transition-colors cursor-pointer">
